...
# NeoVim executable.
VER=v0.3.8 #Update version if needed.
...
if [[ -z $IS_UBUNTU ]]; then
    ...
    sudp yum install lua.x86_64 lua-devel luajit luajit-devel \
        ...
        python34-pip.noarch python-pip.noarch cmake3 make automake gcc gcc-c++
else
    sudo apt update && sudo apt upgrade
    sudo apt install python-dev python3-dev python-pip python3-pip ruby-dev \
        lua5.1 liblua5.1-dev luajit git build-essential cmake3
fi
